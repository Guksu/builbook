...
| 2026-09-24 | **작품 제목 바꾸기**: 작품 목록 카드에 연필 버튼(→ "작품 제목 바꾸기" 창), 작업실 상단 바 제목을 누르면 입력칸(Enter/포커스 아웃=저장, Esc=취소, 빈 제목 무시). `renameProject`는 목록(`projects`)·단일 작품(`project:{id}`) SWR 캐시를 함께 갱신 | entities/project, views/dashboard, widgets/workspace-header, views/workspace, e2e/project-rename.spec.ts | 사용자 요청: "작품 제목도 수정 가능하게" — 문답 결정(두 곳 모두, 목록은 연필 → 작은 창) |
