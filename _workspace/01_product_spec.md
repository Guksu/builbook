...
## 7. 확장: 집필 도구 4종 세트 (2026-07-29)

본격 집필 진입 전에 추가한 실무 도구. 진입장벽 원칙은 그대로다 — 전부 **선택**이고, 쓰지 않으면 보이지 않는다(목표·사전·연표는 비어 있으면 안내 한 줄뿐).

| 세트 | 기능 | 진입점 | 데이터 |
|------|------|--------|--------|
| 데이터 안전 | 전체 백업(JSON) 내보내기 / 복원(합치기·전체교체), 미백업 배너 | 대시보드 "백업" | 전 스토어 |
| 연재 실전 | 오늘 분량·연속 집필일·14일 추이·완성 예상일, 회차 분량표(기본 5,500자), 독자 뷰 미리보기 | 작업실 "현황"·"미리보기" | `writingLogs`(신규), `Project.dailyGoal/episodeGoal` |
| 구조 설계 | 코르크보드(시놉시스 카드·드래그 재정렬·진행 상태 초고/퇴고/완료), 타임라인(사건↔회차 연결) | 작업실 "카드"·"연표" | `events`(신규), `Document.status` |
| 설정 일관성 | 고유명사 사전(정본·이명), 표기 흔들림 검사, 문장 진단(길이·어미 반복·대사 비율) | 작업실 "점검" | `terms`(신규) |

**설계 원칙 2가지**
1. **판단은 작가가 한다.** 표기 검사·문장 진단은 자동 교정하지 않고 "여기 이렇게 적혀 있다"까지만 보여준다.
2. **집계는 단일 출처에서.** 단어/글자 세기는 `@shared/lib`(countWords·countChars·countCharsWithSpaces) 한 곳, 집필량은 자동저장의 저장 델타 한 곳에서만 만들어진다.

세부 구현·주의사항: `docs/worklogs/2026-07-29-writing-tools.md`.
