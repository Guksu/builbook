...
- **인물 관계도** — 헤더의 "관계" 보기. 바인더의 인물 카드가 노드로 나타나고, 끌어서 배치하고, 노드 오른쪽 손잡이에서 다른 인물로 끌면 관계선이 생긴다. 관계에는 종류(가족·연인·적·동료·스승·라이벌 또는 직접 입력)와 방향별 한 줄(A→B, B→A), 메모, **회차별 변화**("3화에서 동맹이 된다"). 선은 종류별 색(작가가 종류마다 바꿀 수 있음), 노드에는 인물 카드의 라벨 색 띠, 라벨이 겹치면 자동으로 비켜남. 도표/목록(표) 전환, "시점" 선택(회차 또는 회차에 연결된 연표 사건)으로 어느 시점까지의 관계인지 보기, 연표 패널에 "관계 변화" 절, "이미지 저장"으로 PNG 내려받기. 배치와 관계는 작품에 저장되고 백업에 담긴다. 라이브러리 없이 SVG로 그린다.
